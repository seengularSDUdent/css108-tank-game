...
class Map{
  ...
  private static void checkMapElement(char[][] checkMap, int numLines, int numColumns)throws InvalidMapException{
    ...
    for(int i = 0; i < numLines; i++){
      for(int j = 0; j < numColumns; j++){
        if(checkMap[i][j] != '0' && checkMap[i][j] != '1' && checkMap[i][j] != 'P' && checkMap[i][j] != 'S' && checkMap[i][j] != 'W' && checkMap[i][j] != 'T' && checkMap[i][j] != 'B'){
  ...
  public void changeElementAt(int i, int j, char z) {
	  this.mainMap[i][j] = z;
  }
}
...
class MyPlayer implements Player{
  ...
  public void setPanes(GridPane gridPane, Pane root, GridPane bulletField) {
	  bullet = new Bullet(gridPane, root, bulletField);
  }
  
  public Bullet getBulletClass() {
	  return this.bullet;
  }
  ...
  @Override
  public void makeFire() {
	  bullet.doFire(currentPosition, playerDirection, currentMap);
  }
